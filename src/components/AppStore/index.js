...
class AppStore extends Component {
  state = {appCategory: tabsList[0].tabId, searchInput: ''}
  ...
  onChangeCategory = tabId => {
    this.setState({appCategory: tabId})
  }

  render() {
    const {searchInput} = this.state
    const {appCategory} = this.state
    const filteredResults = appsList.filter(
      eachItem => eachItem.category === appCategory,
    )

    const searchResults = filteredResults.filter(eachItem =>
        ...
        <div className="store-container">
            ...
            <ul className="tabItems">
              {tabsList.map(eachTabItem => (
                <TabItem
                  ...
                  isActive={appCategory === eachTabItem.tabId}
                />
              ))}
            </ul>
          </div>
          <div className="app-items-container">
            <ul className="app-items">
              {searchResults.map(eachAppItem => (
                <AppItem key={eachAppItem.appId} appItemDetails={eachAppItem} />
              ))}
            </ul>
          </div>
        </div>
